...
def get_inTime():
     try:
         inputTime = input('Enter a time to get all packages till that time\n and enter as (HH:MM:SS): ')
         if inputTime != "Quit" or inputTime !="q" or inputTime != "exit":
             ...
             for count in range(1, 41):
                 ...
                 if converted_first_time >= convert_user_time:
                     ...
                     getAllPackages().get_value(str(count))[9] = "Leaves at "+first_time
                     
                     #Print packages current info
                     print( f'Package ID: {getAllPackages().get_value(str(count))[0]}, ' f'Delivery status: {getAllPackages().get_value(str(count))[10]}')
                 ...
                 # Determine which packages have left left but have not been delivered
                     ...
                     if convert_user_time <converted_second_time:
                         ...
                         getAllPackages().get_value(str(count))[9] = "Left at " + first_time

                         # Print packages current info
                         print( f'Package ID: {getAllPackages().get_value(str(count))[0]}, ' f'Delivery status: {getAllPackages().get_value(str(count))[10]}')
                     else:
                         getAllPackages().get_value(str(count))[10] = "Delievered at " + second_time
                         getAllPackages().get_value(str(count))[9] = "Left at " + first_time
       ...
       if converted_first_time >= convert_user_time:
           ...
           if convert_user_time < converted_second_time:
               ...
               print(
                ...
                getAllPackages().get_value(str(count))[10] = 'Delivered at ' + second_time
                getAllPackages().get_value(str(count))[9] = "Left at "+ first_time  
...
def get_by_city():
    ...
    if user_input is not "Quit" or not "q" or not "quit":
        for count in range(1,41):
            ...
            (hrs, mins, secs) = user_time.split(':')
            convert_user_time = datetime.timedelta(hours=int(hrs), minutes=int(mins), seconds=int(secs))
            print(f' this is count {count} and line for {getAllPackages().get_value(str(count))[6]}')
...
def get_within_range():
    user_first_range = input("Input the first time range: ")
    user_end_range = input("Input the end time range: ")
    ...
    if user_first_range and user_end_range !="Quit" or not "q" or not "quit" or not "exit":
        for count in range(1,41):
            ...
            con_first_range = datetime.timedelta(hours=int(hrs), minutes=int(mins), seconds=int(secs))
            (hrs, mins, secs) = user_end_range.split(':')
            con_end_range = datetime.timedelta(hours=int(hrs), minutes=int(mins), seconds=int(secs))

            if converted_first_time>=con_first_range or converted_second_time < con_end_range:
