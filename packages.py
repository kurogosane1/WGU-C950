...
for index in range(len(distance.first_truck_index())):
   try:
       ...
       # All delivering packages
       del_package = distance.get_truck_time(distance.currentlyTravelled(int(distance.first_truck_index()[index]), int(distance.first_truck_index()[index+1])),first_truck_leave_time)
       distance.first_truck_lists()[index][10] = (str(del_package))
